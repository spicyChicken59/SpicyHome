import {
  ...
  spicyPicks,
  pickLenses,
} from "./model.js?v=20260908-picks";
...
function exploreToolbar() {
  return `<section class="explore-deck" aria-label="Explore apartments"><div class="explore-heading"><div><p class="eyebrow">MAKE ROOM FOR WHAT MATTERS</p><strong id="search-pulse"></strong><p id="search-scope" class="meta"></p></div><button class="text-button" id="open-search-controls">Adjust budget &amp; area ↗</button></div><div class="explore-switches"><div class="segmented bedroom-shortcuts" aria-label="Bedrooms">${[["all","1 & 2 beds"],["1","1 bed"],["2","2 beds"]].map(([value,label]) => `<button data-bed="${value}" aria-pressed="${prefs.bedrooms === value}">${label}</button>`).join("")}</div><div class="segmented surface-switch" aria-label="Explore view">${[["split","List + map"],["list","List"],["map","Map"],["focus","Focus"],["atlas","Atlas"]].map(([value,label]) => `<button data-surface="${value}" aria-pressed="${prefs.surface === value}">${label}</button>`).join("")}</div></div></section>`;
}
...
function updateExploreSurface(homes) {
  ...
  } else if (map) { map.remove(); map = null; markers.clear(); }
...
function renderJumpResults() {
  ...
  const screens=[["discover","Discover apartments"],["shortlist","Shortlist, finalists & tours"],["lab","Cost Lab"],["timeline","Activity"],["setup","Sources & setup"]].filter(([,label])=>label.toLowerCase().includes(query));
...
function pickCard(pick,index) {
  ...
  const verdict=pickLens==="ev" ? "EV lifestyle fit" : pickLens==="rail" ? "Close to CTA" : pickLens==="space" ? "Room for the money" : pick.saving!==null && pick.saving>=.05 ? "Local price signal" : pickLens==="budget" ? "Budget room" : "Balanced fit";
  const leading=pickLens==="ev" ? pick.reasons.filter((reason)=>/parking|charging/i.test(reason)) : pickLens==="rail" ? pick.reasons.filter((reason)=>/CTA station/.test(reason)) : pickLens==="space" ? pick.reasons.filter((reason)=>/reported sq ft/.test(reason)) : [];
