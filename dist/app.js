...
} from "./model.js?v=20260919-quote-dates";
...
function renderCard(h) {
  ...
  return `<article class="home-card" data-home="${esc(h.id)}">
    ...
    <div class="property-evidence"><p class="meta layout-evidence">${esc(layout.label)}</p><div class="chips">${chip(h, "parking", "Parking")}${chip(h, "charging", "EV")}${formChip(h)}</div>${bandLine(h)}<div class="cost-line"><span>Known monthly subtotal</span><strong>${subtotalValue(c)}</strong></div>${costGap(h, c)}</div>
...
function showDetail(id) {
  ...
  $("#detail-content").innerHTML =
    ...
    <div class="dossier-subtotal"><span>Known monthly subtotal <strong>${subtotalValue(c)}</strong></span><span>${c.unknown.length ? "Incomplete · " + esc(c.unknown.join(", ")) + " unquoted" : "Every monthly subtotal item entered"} · EV charging separate</span></div>
