import {
  ...
  scenarioFields,
  costScenario,
} from "./model.js?v=20260908-experience";
...
let lab = { homeId: null, assumptions: {}, months: 12 };
try {
...
function resetSearchFilters(keepLayout = false) {
  ...
  prefs = { ...defaults, utilityEstimate: prefs.utilityEstimate, surface: prefs.surface, layoutScope, bedrooms };
  persist();
  render();
  $("#reset-filters")?.focus();
}
...
function bindCards() {
  ...
  document.querySelectorAll("[data-lab]").forEach((b) => { b.onclick = () => { lab.homeId = b.dataset.lab; lab.assumptions = {}; view = "lab"; render(); $("#lab-home")?.focus(); }; });
...
function exploreToolbar() {
  return `<section class="explore-deck" aria-label="Explore apartments"><div class="explore-heading"><div><p class="eyebrow">MAKE ROOM FOR WHAT MATTERS</p><strong id="search-pulse"></strong><p id="search-scope" class="meta"></p></div><button class="text-button" id="open-search-controls">Adjust budget &amp; area ↗</button></div><div class="explore-switches"><div class="segmented bedroom-shortcuts" aria-label="Bedrooms">${[["all","1 & 2 beds"],["1","1 bed"],["2","2 beds"]].map(([value,label]) => `<button data-bed="${value}" aria-pressed="${prefs.bedrooms === value}">${label}</button>`).join("")}</div><div class="segmented surface-switch" aria-label="Explore view">${[["split","List + map"],["list","List"],["map","Map"],["focus","Focus"]].map(([value,label]) => `<button data-surface="${value}" aria-pressed="${prefs.surface === value}">${label}</button>`).join("")}</div></div></section>`;
}
...
function updateExploreSurface(homes) {
  ...
  layout.hidden = prefs.surface === "focus";
...
function renderFocus(homes) {
  ...
  $("#focus-undo").onclick = () => {
    ...
    else { state.records[last.id] = { ...record(last.id), saved: last.saved }; event(last.id, "Undid the last Focus save."); saveNotice(persist()); }
...
function renderLab() {
  ...
  const updateScenario = () => {
    ...
    lab.assumptions = Object.fromEntries(inputs.map((input) => [input.dataset.scenario, input.value === "" ? null : Number(input.value)]));
    lab.months = Number($("#lab-months").value);
    $("#lab-month-value").textContent = lab.months;
    renderLabOutput(home);
  };
  document.querySelectorAll("[data-scenario]").forEach((input) => { input.oninput = updateScenario; });
  $("#lab-months").oninput = updateScenario;
  $("#lab-reset").onclick = () => { lab.assumptions = {}; lab.months = 12; renderLab(); bindCards(); $("#lab-rent").focus(); };
  renderLabOutput(home);
}
