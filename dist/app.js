import {
  ...
  openQuestions, figureSpread, surfacedBecause, headlineUnknown,
  buildingForm, urbanSetting, urbanBands, budgetBand, parkingStanding, doubleDownOn, focusUnknowns, lensPresets,
  quoteEvidence, recordQuote,
  eligibilityReading,
...
function savedScan(id, home, keepOriginal = false) {
  const prior = record(id).scan;
  if (keepOriginal || !home || home.notebook_only) return prior;
  const basis =
...
// A notebook action must not backfill a frozen snapshot with later evidence.
// Explicitly saved later evidence has its own recording time, separate from
// the source's observation day and the original snapshot/query dates.
function savedSource(id, home) {
  const prior = record(id);
  const keepOriginal = !!prior.snapshot && !!(home?.home_evidence || prior.snapshot.home_evidence || prior.home_evidence_update || home?.eligibility_evidence || prior.snapshot.eligibility_evidence || prior.eligibility_update);
...
function eligibilitySummary(h, inDossier = false) {
  const reading = eligibilityReading(h, record(h.id));
  if (!reading.evidence) return "";
  const action = inDossier ? 'data-detail-jump="detail-eligibility"' : `data-studio-task="${esc(h.id)}" data-task-target="detail-eligibility"`;
  return `<p class="eligibility-note">${reading.current ? "" : '<span class="meta">Saved source evidence · </span>'}${esc(reading.evidence.note)} <button class="text-button" ${action}>Source &amp; date ↗</button></p>`;
}
function eligibilityDetail(h, r) {
  const reading = eligibilityReading(h, r);
  const block = (e, label) => `<div class="eligibility-source"><p class="eyebrow">${esc(label)}</p><p>${esc(e.note)}</p><p class="meta">Scope: ${e.scope === "address" ? "address / program; exact offer unconfirmed" : "source-identified offer; your eligibility unconfirmed"}. Source observed ${esc(dateLabel(e.source.observed_at))}.</p><p>${esc(e.source.supports)}</p>${link(e.source.url, e.source.name + " ↗")}</div>`;
  let body = reading.evidence ? block(reading.evidence, reading.context) : '<p>Eligibility evidence is not recorded. This does not establish that the housing is unrestricted.</p>';
  if (r.snapshot) {
    if (!reading.original) body += '<p class="meta eligibility-original">Eligibility evidence was not recorded in your original saved snapshot. That snapshot has not been rewritten.</p>';
    else if (JSON.stringify(reading.original) !== JSON.stringify(reading.evidence)) body += block(reading.original, "Original saved snapshot");
    else body += '<p class="meta eligibility-original">This source evidence is retained in your original saved snapshot.</p>';
    if (r.eligibility_update) {
      if (JSON.stringify(r.eligibility_update.evidence) !== JSON.stringify(reading.evidence)) body += block(r.eligibility_update.evidence, "Separately saved source evidence");
      body += `<p class="meta">Later source evidence recorded in this notebook ${esc(recordedLabel(r.eligibility_update.recorded_at))}; its source observation day is shown separately above.</p>`;
    }
  }
  return `<details class="detail-section eligibility-detail" id="detail-eligibility" tabindex="-1"><summary>Eligibility evidence <span>Source, date &amp; scope</span></summary>${body}</details>`;
}
function eligibilityCell(h) {
  const reading = eligibilityReading(h, record(h.id));
  const e = reading.evidence;
  if (!e) return "Eligibility evidence not recorded. Restrictions unknown.";
  const scope = e.scope === "address" ? "Income-restricted housing advertised at this address; exact offer unconfirmed." : "Income restriction recorded for this offer.";
  return `${scope} Confirm your eligibility. ${reading.current ? "Source" : "Saved source"} observed ${dateLabel(e.source.observed_at)}.`;
}
...
// A reader's own quotes, every one the record carries, newest first. The
// amount, the day it was quoted and the instant the notebook saved it are
// three facts and are printed as three: an entry with no quote date is listed
// as unknown and never dated from the day it was saved, an entry from before
// quote-date provenance was kept says so, and the series is drawn only from
// the days the reader established (quoteEvidence). Nothing here is a source
// observation; those are the chart above it.
function quoteLog(r) {
  const ev = quoteEvidence(r);
  if (!ev.entries.length) return "";
  const when = (e) =>
    e.basis === "unknown"
      ? "quote date unknown — none entered, none assumed"
      : e.basis === "entered"
        ? `quoted ${dateLabel(e.date)}, a date you entered`
        : `dated ${dateLabel(e.date)} — recorded before this notebook kept quote-date provenance, so the day may be when it was saved rather than when you were quoted${e.supported ? "; it matches your dated active quote and counts" : "; it forms no dated movement"}`;
  const fixes = (e) =>
    e.corrections.length
      ? " · Date corrected " +
        e.corrections.map((c) => `${recordedLabel(c.at)} (was ${c.from ? dateLabel(c.from) : "unknown"})`).join("; ")
      : "";
  const items = [...ev.entries]
    .reverse()
    .map((e) => `<li><strong>${e.rent === null ? "Not quoted" : money(e.rent)}</strong> · ${esc(when(e))} · ${e.recorded_at ? "Recorded " + esc(recordedLabel(e.recorded_at)) : "Recording time not kept"}${esc(fixes(e))}</li>`)
    .join("");
  const summary = [
    `${ev.dated.length} of ${ev.entries.length} dated by you`,
    ev.unknown ? `${ev.unknown} with no quote date` : "",
    ev.unrecorded ? `${ev.unrecorded} without date provenance` : "",
  ].filter(Boolean).join(" · ");
  return `<h3>Your recorded quotes</h3>${ev.dated.length >= 2 ? priceChart({ history: ev.dated }) : ""}<ol class="quote-log" reversed>${items}</ol><p class="meta">${esc(summary)}. Only the days you entered form a dated series or a price movement. A recording time is when this notebook saved an entry, never when you were quoted.</p>`;
}
// The instant an entry was saved, in the notebook's own time zone, and said to
// be that: a time, never a quote date.
function recordedLabel(iso) {
  const time = Date.parse(iso);
  return Number.isFinite(time)
    ? new Intl.DateTimeFormat("en-US", { timeZone: "America/Chicago", month: "short", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" }).format(new Date(time)) + " Chicago time"
    : "at a time this notebook did not keep";
}
function heartIcon() {
...
function renderCard(h) {
  const r = record(h.id),
    ...
    delta = changeFor(h),
    saved = !!r.saved;
  return `<article class="home-card" data-home="${esc(h.id)}">
    <div class="card-top"><span class="neighborhood">${esc(homeCity(h) || h.neighborhood)}</span><button class="save-button" data-save="${esc(h.id)}" aria-label="${saved ? "Remove" : "Save"} ${esc(h.title)} ${saved ? "from" : "to"} shortlist" aria-pressed="${saved}">${heartIcon()}</button></div>
    <div class="property-composition"><div class="property-identity"><h3>${esc(h.title)}</h3><p class="address">${esc(h.address)}</p><p class="plan-label">${esc(planLabel(h))} · ${h.sqft ? esc(h.sqft) + " sq ft" : "Size unverified"}</p></div>
    <div class="property-price"><div class="rent-row"><strong class="rent">${money(displayPrice(h))}</strong>${displayPrice(h) !== null ? "<small>/ mo</small>" : ""}</div><p class="price-kind">${esc(priceKind(h))}</p></div></div>
    ${eligibilitySummary(h)}${homeEvidenceSummary(h)}<div class="property-evidence"><p class="meta layout-evidence">${esc(layout.label)}</p><div class="chips">${chip(h, "parking", "Parking")}${chip(h, "charging", "EV")}${formChip(h)}</div>${bandLine(h)}<div class="cost-line"><span>Known monthly subtotal</span><strong>${subtotalValue(c)}</strong></div>${costGap(h, c)}</div>
    ${delta !== null && delta !== 0 ? `<div class="price-change">${delta < 0 ? "↓" : "↑"} ${money(Math.abs(delta))} since previous observation</div>` : ""}${r.tourDate ? `<p class="tour-note">Tour: ${esc(r.tourDate.replace("T", " · "))}</p>` : ""}${h.notebook_only ? '<p class="price-change">Archived notebook entry · absent from the current feed</p>' : h.seen_in_latest === false ? '<p class="price-change">Not seen in this area’s last scan · availability unverified</p>' : ""}
    <div class="evidence-stamp">${evidence(h)} · ${esc(dateLabel(h.observed_at))} · ${esc(sourceAccess(h).summary)}</div>${r.status ? `<p class="meta card-status">${esc(r.status)}</p>` : ""}${tourProgress(r) ? `<p class="tour-progress">Your tour review · ${tourProgress(r)}/${tourChecks.length} checked</p>` : ""}
    <div class="card-actions"><button class="button small secondary" data-detail="${esc(h.id)}">Open dossier <span aria-hidden="true">↗</span></button><label class="compare-label"><input type="checkbox" data-compare="${esc(h.id)}" ${comparison.has(h.id) ? "checked" : ""}>Compare</label><button class="text-button" data-tour="${esc(h.id)}">Tour companion ↗</button></div></article>`;
}
...
function render() {
  ...
  if (!feed) return;
  const names = {
    discover: ["THE SEARCH", "Places to consider."],
    shortlist: ["YOUR SAVED PLACES", "The Decision Desk."],
...
function renderDiscover() {
  ...
  $("#urban-scope").onchange = () => {
    prefs.urbanScope = $("#urban-scope").value;
    persist(); renderResults(); bindCards();
  };
  ["highRise", "parkingPreferred"].forEach((key) =>
    $("#filter-" + key).addEventListener("change", () => {
      prefs[key] = $("#filter-" + key).checked;
      persist(); renderResults(); bindCards();
    }));
...
function updateFilters() {
  ...
  if (target !== null && (!Number.isFinite(target) || target < 0 || target > 20000)) {
    toast("Enter a target between $0 and $20,000, or leave it blank.");
    return false;
  }
  prefs = {
    ...prefs,
...
// What the record itself recorded about the building, and where the words came
// from. A record nothing describes says exactly that: this is never drawn as a
// building that is not tall, because nobody wrote down that it is not.
function formLine(h) {
  const form = buildingForm({ ...h, home_evidence: homeEvidenceReading(h, record(h.id)).evidence });
  return `<p class="meta detail-form"><span class="form-mark form-mark--${esc(form.status)}">${esc(form.short)}</span> ${esc(form.because)}${form.quote ? ` <q class="form-quote">${esc(form.quote)}</q>` : ""}${form.observed_at ? ` Recorded ${esc(dateLabel(form.observed_at))}.` : ""}</p>`;
}
function formChip(h) {
  const form = buildingForm({ ...h, home_evidence: homeEvidenceReading(h, record(h.id)).evidence });
  return form.status === "unknown" ? "" : `<span class="chip form">${esc(form.short)}</span>`;
}

// The reader's own target, on the basis their own budget filter is set to. It
// is never presented as an all-in cost and a place above it is named as above
// it -- shown so it can be weighed, not recommended.
function bandLine(h) {
  if (prefs.budgetMode === "flexible") return `<p class="meta budget-band">${esc(preferredBudget(h, record(h.id), prefs))}</p>`;
  const band = budgetBand(h, record(h.id), prefs);
  return ["off", "unpriced"].includes(band.status) ? ""
    : `<p class="band-line band-line--${esc(band.status)}"><span>${esc(band.label)}</span><span class="meta">${esc(band.caveat)}</span></p>`;
}
// The counts a reader needs to judge what the lens can and cannot tell them,
// over the records actually retained. Every sentence is a count of this feed,
// and the one about missing evidence says what it means: nothing recorded is
// not a finding that nothing is there.
function downtownSummary() {
  const el = $("#downtown-lens");
  if (!el) return;
  if (homeSearchActive(prefs)) { el.hidden = true; el.replaceChildren(); return; }
  el.hidden = !doubleDownOn(prefs);
  if (el.hidden) { el.replaceChildren(); return; }
  const pool = allHomes().filter((h) => !h.notebook_only);
  const scope = pool.filter((h) => {
    const status = urbanSetting(h, feed).status;
    return prefs.urbanScope === "core" ? status === "core"
      : prefs.urbanScope === "near" ? ["core", "near"].includes(status) : true;
  });
  const anchor = urbanSetting(pool[0] ?? {}, feed).anchor ?? "central Chicago";
  const where = prefs.urbanScope === "core" ? `within ${urbanBands.core} straight-line mile of ${anchor}`
    : prefs.urbanScope === "near" ? `within ${urbanBands.near} straight-line miles of ${anchor}`
      : "across the whole search area";
  const tally = (fn) => scope.reduce((all, h) => { const key = fn(h); all[key] = (all[key] ?? 0) + 1; return all; }, {});
  const forms = tally((h) => buildingForm(h).status);
  const parked = tally((h) => parkingStanding(h, record(h.id)).status);
  const lines = [`<p><strong>${scope.length}</strong> retained record${scope.length === 1 ? "" : "s"} ${where}.</p>`];
  if (prefs.highRise)
    lines.push(`<p>${forms.high_rise ?? 0} of them ${(forms.high_rise ?? 0) === 1 ? "has a building height in its own sources" : "have a building height in their own sources"}${forms.low_mid_rise ? `, ${forms.low_mid_rise} recorded as low or mid-rise` : ""}. ${forms.unknown ?? 0} have none recorded${(forms.high_rise ?? 0) === 0 ? " — so no record here carries enough evidence to call its building a high-rise. That is what these sources say, not a finding that there are none" : ""}. A height nobody wrote down is not a low building.</p>`);
  const target = amount(prefs.targetRent);
  if (target !== null) {
    const bands = tally((h) => budgetBand(h, record(h.id), prefs).status);
    const over = scope.map((h) => costs(h, record(h.id), prefs).rent).filter((rent) => rent !== null && rent > prefs.max).sort((a, b) => a - b);
    lines.push(`<p>Against your ${money(target)} target, on ${prefs.basis === "total" ? "the known monthly subtotal" : "base rent"}: ${bands.under ?? 0} under it, ${bands.near ?? 0} around it, ${bands.stretch ?? 0} above it and inside your ${money(prefs.max)} cap${bands.unpriced ? `, ${bands.unpriced} with no quoted figure at all` : ""}. None of these is an all-in monthly cost.</p>`);
    if (over.length)
      lines.push(`<p>${over.length} more ${over.length === 1 ? "sits" : "sit"} past your ${money(prefs.max)} cap, the nearest at ${money(over[0])}. They are not hidden because they are worse — raise the maximum above to weigh them.</p>`);
    // Each of these is a fact about what this snapshot retained, never a claim
    // that the area has nothing: a capped query and a lens both leave things out.
    const priced = (bands.under ?? 0) + (bands.near ?? 0) + (bands.stretch ?? 0) + (bands.outside ?? 0);
    if (!priced)
      lines.push(`<p><strong>No record in this area has a quoted base rent to put against your target.</strong> That is what these sources carry, not a finding that nothing is available here.</p>`);
    else if (!(bands.near ?? 0) && !(bands.under ?? 0))
      lines.push(`<p><strong>This retained snapshot holds nothing at or under your ${money(target)} target in this area.</strong> The ${bands.stretch ?? 0} above it ${(bands.stretch ?? 0) === 1 ? "is" : "are"} shown as stretches, and ${over.length ? "more sit past your cap" : "widening the area or the target would take in more"}.</p>`);
    else if (!(bands.near ?? 0))
      lines.push(`<p>Nothing here sits <em>around</em> your target: ${bands.under ?? 0} ${(bands.under ?? 0) === 1 ? "is" : "are"} under it and ${bands.stretch ?? 0} above.</p>`);
  }
  lines.push(`<p>${(parked.advertised ?? 0) + (parked.priced ?? 0)} advertise resident parking${parked.priced ? ` (${parked.priced} with a price on record)` : ""}; ${parked.unknown ?? 0} have nothing recorded either way, and ${parked.none ?? 0} report none. Advertised is not a reserved space.</p>`);
  el.innerHTML = `<p class="why-label">What this lens can see</p>${lines.join("")}`;
}
function renderResults() {
  $("#saved-count").textContent = Object.values(state.records).filter((r) => r.saved).length;
  const loaded = allHomes(),
    homes = visibleHomes(loaded.filter((h) => !h.notebook_only), state, prefs, feed),
  ...
  summary.hidden = homeSearchActive(prefs) || hiddenListings === 0;
  summary.innerHTML = hiddenListings
    ...
    : "";
  if (hiddenListings && $("#show-unfiltered")) $("#show-unfiltered").onclick = resetSearchFilters;
  ...
  $("#results").innerHTML = homes.length
    ...
    : homeSearchActive(prefs)
      ? empty(prefs.resultGroup === "leads" ? "No known-area verification leads in this view." : "No records support every selected property criterion yet.",
          "This describes retained evidence, not the apartment market. Review the named gaps in verification leads, or explicitly change a criterion. Unknown neighborhoods stay in their own group.",
          '<button class="button secondary" id="edit-home-criteria">Edit selected criteria</button>')
    : prefs.layoutScope === "confirmed" && !checked
      ...
      : prefs.urbanScope !== defaults.urbanScope
        ? empty("Nothing in this retained snapshot sits inside the downtown lens and your other filters.",
            `The lens keeps the ${prefs.urbanScope === "core" ? urbanBands.core + "-mile core" : urbanBands.near + " miles"} around the centre this search recorded, and a place with no recorded coordinates is outside any distance. This says what these records hold, not that the area is empty.`,
            '<button class="button secondary" id="widen-lens">Look anywhere in the search</button>')
  ...
  if ($("#edit-home-criteria")) $("#edit-home-criteria").onclick = () => { homeSearchControlsOpen = true; $("#home-search-editor").open = true; $("#home-search-editor summary").focus(); };
  ...
  if ($("#widen-lens")) $("#widen-lens").onclick = () => { prefs.urbanScope = defaults.urbanScope; persist(); render(); $("#urban-scope")?.focus(); };
...
function renderMap(homes) {
  ...
  // A zoom animation finishes on a transitionend up to a quarter-second later.
  // The fit a RE-RENDER performs has nothing to animate from -- the map was
  // built moments ago -- and if the next re-render arrives first, that pending
  // handler reads panes its map no longer has and throws inside Leaflet, where
  // the page cannot catch it. Changing a filter while the panel is opening does
  // exactly that. The reader's own "Fit all homes" stays animated.
  const fitHomes = (animate = true) => {
    if (placed.length) map.fitBounds(placed.map(h => [h.lat, h.lng]), { padding: [45, 35], maxZoom: 14, animate });
    else map.setView([41.882, -87.632], 11, { animate });
  };
  if (keepCamera && mapCamera) map.setView(mapCamera.center, mapCamera.zoom, { animate: false });
  else fitHomes(false);
  mapContentKey = contentKey;
  $("#map-fit").onclick = () => fitHomes(true);
...
function renderShortlist() {
  ...
  document.querySelectorAll("[data-stage]").forEach((select) => { select.onchange = () => { const id = select.dataset.stage; state.records[id] = { ...record(id), status: select.value, ...savedSource(id, getHome(id)), saved: true }; event(id, "Moved to " + select.value + "."); const ok = persist(); render(); focusHomeControl(id, "stage"); saveNotice(ok); }; });
  bindFinalists();
  renderTray();
}
...
function comparisonEntries() {
  const loaded = allHomes();
  const shown = new Set(visibleHomes(loaded.filter((h) => !h.notebook_only), state, prefs, feed).map((h) => h.id));
...
function showDetail(id) {
  ...
  document.querySelectorAll("[data-tour-check]").forEach((input) => { input.onchange = () => { const done = document.querySelectorAll("[data-tour-check]:checked").length; $("#tour-draft-count").textContent = `${done}/${tourChecks.length} reviewed · ${tourChecks.length - done} left · save changes to keep`; }; });
  document.querySelectorAll("#detail-content [data-close]").forEach(button => { button.onclick = () => $("#detail-dialog").close(); });
  $("#record-form").onsubmit = (e) => {
    ...
    Object.assign(next, savedSource(id, h));
    // The quote history is written by recordQuote() and by nothing else: a
    // changed amount is an observation on the day entered -- or on no day,
    // said so, never on the day it was saved -- a changed date alone corrects
    // the active quote's own entry, and a save that touched notes, a stage or
    // a tour records nothing and re-dates nothing.
    const quote = recordQuote(r, next, new Date().toISOString());
    if (quote.change) next.quote_history = quote.history;
    state.records[id] = next;
    event(
      id,
      "Updated notes and quotes · " + next.status + "." +
        (quote.change === "observed"
          ? ` Recorded a ${money(next.rentOverride)} base-rent quote, ${next.quoteDate ? "quoted " + dateLabel(next.quoteDate) : "quote date unknown"}.`
          : quote.change === "corrected"
            ? ` Corrected that quote's date to ${next.quoteDate ? dateLabel(next.quoteDate) : "unknown"}.`
            : ""),
    );
    const ok = persist();
    $("#detail-dialog").close();
...
function showCompare() {
  ...
  const rows = [
    ...
    ["Building form on record", (h) => buildingForm({ ...h, home_evidence: homeEvidenceReading(h, record(h.id)).evidence }).short],
    ["Selected home criteria", h => criteriaStatusText(h), true],
    ["Private balcony", h => homeAttributeText(h, "balcony"), true],
    ["In-unit washer AND dryer", h => homeAttributeText(h, "laundry"), true],
    ["Scoped area (sq ft)", h => homeAttributeText(h, "sqft"), true],
    ["Actual apartment floor", h => homeAttributeText(h, "floor")],
    ["Actual apartment view", h => homeAttributeText(h, "view")],
    ["Target move-in", h => moveInReading(h, record(h.id), prefs.moveIn).label, true],
    ["Preferred range", h => prefs.budgetMode === "flexible" ? preferredBudget(h, record(h.id), prefs) : "Strict search bounds apply", true],
    ["Resident parking", (h) => parkingStanding(h, record(h.id)).label],
    ["Base rent", (h) => money(costs(h, record(h.id), prefs).rent), true],
    ["Eligibility evidence", eligibilityCell, true],
    [
...
$("#import-file").onchange = async (e) => {
  ...
  try {
    ...
    if (
      !confirm(
        "Import this notebook? Incoming search settings, notes and named searches replace matching entries. Export your current notebook first if you need a backup.",
      )
    )
      return;
    const byId = new Map(
    ...
    const merged = validateWorkspace({
      ...
      searchUndo: incoming.searchUndo ?? null,
...
function updateExploreSurface(homes) {
  ...
  if (prefs.budgetMode === "flexible") $("#search-scope").textContent = `${prefs.bedrooms === "all" ? "1–2 bedroom choices" : prefs.bedrooms + " bedroom"} · Preferred ${money(prefs.preferredMin)}–${money(prefs.preferredMax)} · ${prefs.strictCap === null ? "Flexible, no strict cap" : "Explicit cap " + money(prefs.strictCap)}`;
...
function renderFocus(homes) {
  ...
  if (home) {
    ...
    $("#focus-save").onclick = () => {
      ...
      state.records[home.id] = { ...previous, saved: true, status: previous.status ?? "shortlisted", ...savedSource(home.id, home) };
...
// What the record already says about the thing a check asks the reader to go and
// look at, beside that check. Nothing is invented and nothing is marked
// verified: an unknown stays unknown, a recorded $0 stays an amount, and the
// building's own charging answer stays separate from its cost and from any
// public station.
function tourCheckEvidence(key, home, rec) {
  const cost = costs(home, rec, prefs);
  if (key === "layout") return `On record: ${layoutEvidence(home, rec).label}.`;
  if (key === "parking") {
    const status = { yes: "Parking is advertised here", no: "No parking is offered here, according to the source",
      unknown: "The source never established whether there is parking" }[home.parking?.status ?? "unknown"];
    return `${status}. Monthly cost on record: ${cost.parking === null ? "not quoted" : money(cost.parking)}.`;
  }
  if (key === "charging") {
    const ev = chargingEvidence(home, feed);
    return `${ev.building}. Its cost is quoted by no one here and is not in the subtotal. ${
      ev.public === "unavailable" ? "No public charging dataset is loaded, which is not evidence that there are none nearby." : "Nearby public stations are not a resident amenity."}`;
  }
  if (key === "lease")
    return `Known monthly subtotal on record: ${money(cost.known)}${cost.unknown.length ? ` — still unquoted: ${cost.unknown.join(", ")}.` : ", with every monthly item entered."}`;
  return "";
}
// The walkthrough a reader actually holds at the door: which apartment this is,
// when the tour is, how much is left, and each check with what the record
// already knows about it. It sits directly above the fields that record the
// answers, so a check and the place to write down what it found are one short
// scroll apart rather than four screens.
function tourCompanion(home, rec) {
  const reviewed = tourProgress(rec), total = tourChecks.length;
  const when = rec.tourDate ? chicagoTime(rec.tourDate) : null;
  return `<details id="tour-companion" class="tour-companion"><summary>Tour companion <span id="tour-draft-count">${reviewed}/${total} reviewed · ${total - reviewed} left</span></summary><div class="tour-head"><div><strong>${esc(home.title)}</strong><span>${esc(planLabel(home))} · ${esc(home.address)}</span></div><div class="tour-when">${when ? `<strong>${esc(dateLabel(when.slice(0, 10)))}</strong><span>${esc(when.slice(11))} · Chicago</span>` : '<span>No tour date recorded</span>'}</div>${link("https://www.google.com/maps/dir/?api=1&destination=" + encodeURIComponent(home.address), "Directions ↗", "text-button")}</div><p class="meta">A check means you reviewed it — not that anyone certified it.</p><div class="tour-checklist">${tourChecks.map(([key,label,prompt],index) => {
    const known = tourCheckEvidence(key, home, rec);
    return `<label class="tour-check"><input type="checkbox" name="tour:${key}" data-tour-check="${key}" form="record-form" ${rec.tourChecks?.[key] ? "checked" : ""}><span><strong>${String(index+1).padStart(2,"0")} / ${esc(label)}</strong><span>${esc(prompt)}</span>${known ? `<span class="tour-known">${esc(known)}</span>` : ""}</span></label>`;
  }).join("")}</div><p class="meta">Write what you found in the fields below: a quote and its date, the parking and fee amounts, and your own notes. Your checks save with them.</p></details>`;
}
function renderLab() {
...
function refreshSearchCounts() {
  ...
  document.querySelectorAll("[data-preset-count]").forEach((span) => {
    const preset = lensPresets.find((entry) => entry.key === span.dataset.presetCount);
    if (!preset) return;
    // The count a press would produce, measured the way the page measures every
    // other one: the preset merged onto what the reader already has.
    const merged = { ...prefs, ...preset.preferences };
    span.textContent = `${visibleHomes(homes, state, merged, feed).length} places now \u00b7 ${esc(preset.note)}`;
  });
  document.querySelectorAll("[data-search-count]").forEach((span) => {
    const search = state.savedSearches[Number(span.dataset.searchCount)];
    if (search) span.textContent = `${visibleHomes(homes, state, {...search.preferences, utilityEstimate: prefs.utilityEstimate}, feed).length} matches now · ${search.preferences.budgetMode === "flexible" ? "Flexible preferred range" : money(search.preferences.max) + " cap"}`;
  });
}
function bindSearchShelf() {
  ...
  document.querySelectorAll("button[data-preset]").forEach((button) => {
    button.onclick = () => {
      const preset = lensPresets.find((entry) => entry.key === button.dataset.preset);
      if (!preset) return;
      prefs = { ...prefs, ...preset.preferences };
      savedSearchesOpen = true;
      const ok = persist();
      render();
      $(`button[data-preset="${preset.key}"]`)?.focus();
      saveNotice(ok);
    };
  });
...
function renderScanCard(home) {
  ...
  return `<article class="home-card scan-card" data-home="${esc(home.id)}"><div class="scan-identity"><p class="neighborhood">${esc(homeCity(home) || home.neighborhood)}</p><h3>${esc(home.title)}</h3><p class="plan-label">${esc(planLabel(home))}</p><p class="address">${esc(home.address)}</p><p class="meta">${esc(layout.label)} · ${home.sqft ? home.sqft + " sq ft reported" : "Size unverified"}</p></div><div class="scan-price"><strong>${money(displayPrice(home))}<small> / mo</small></strong><p class="price-kind">${esc(priceKind(home))}</p><p class="meta">Known subtotal: ${subtotalValue(cost)}</p>${costGap(home, cost)}<p class="meta">${evidence(home)} · ${esc(dateLabel(home.observed_at))}${home.seen_in_latest === false ? " · Not in last area scan" : ""}</p></div>${eligibilitySummary(home)}${homeEvidenceSummary(home)}<div class="scan-actions"><button class="button small secondary" data-detail="${esc(home.id)}">Details</button><button class="save-button" data-save="${esc(home.id)}" aria-label="${rec.saved ? "Remove" : "Save"} ${esc(home.title)} ${esc(planLabel(home))}" aria-pressed="${!!rec.saved}">${heartIcon()}</button><label class="compare-label"><input type="checkbox" data-compare="${esc(home.id)}" ${comparison.has(home.id) ? "checked" : ""}>Compare</label></div></article>`;
}
...
function finalistShelf(saved, groups) {
  ...
  return `<section class="finalist-shelf${finalists.length ? "" : " is-empty"}" aria-label="Your finalists"><div class="finalist-heading"><div><p class="eyebrow">THE FINAL THREE</p><h3>${finalists.length ? "Imagine life in each." : "Narrow it to three."}</h3></div><span>${finalists.length}/3 pinned</span></div>${finalists.length ? `${finalistDifference(finalists)}<div class="finalist-grid">${finalists.map((home, i) => {
    const rec = record(home.id), cost = costs(home, rec, prefs), unresolved = open(home);
    const move = nextMove(home, state, prefs);
    return `<article class="finalist-candidate" data-finalist-home="${esc(home.id)}">
      <div class="finalist-location"><span class="finalist-number" aria-label="Candidate ${String.fromCharCode(65+i)}">${String.fromCharCode(65+i)} / YOUR FINALIST</span><span>${esc(homeCity(home) || home.neighborhood)}</span></div>
      <div class="finalist-top"><div><h4>${esc(home.title)}</h4><p class="finalist-plan">${esc(planLabel(home))}</p><p class="finalist-address">${esc(home.address)}</p></div></div>
      <p class="finalist-rent"><strong>${money(displayPrice(home))}</strong><span>${esc(priceKind(home))}${displayPrice(home) !== null ? " / month" : ""}</span></p>
      ${eligibilitySummary(home)}<dl class="finalist-facts"><div data-fact="wide"><dt>Known subtotal</dt><dd>${subtotalValue(cost)}</dd></div><div data-fact="wide"><dt>Layout</dt><dd>${esc(layoutEvidence(home, rec).label)}</dd></div><div data-fact="wide"><dt>Reported space</dt><dd>${home.sqft ? esc(home.sqft) + " sq ft" : "Not recorded"}</dd></div><div data-fact="always"><dt>Unresolved</dt><dd>${unresolved ? `${unresolved} open` : "none on record"}</dd></div></dl>
      <p class="finalist-missing">${cost.unknown.length ? `Still unquoted: ${esc(cost.unknown.join(", "))}. The subtotal is incomplete.` : "Every monthly subtotal item is entered."} ${home.charging?.status === "no" ? "Resident EV charging is not offered." : "Resident EV charging is not included."}</p>
      <p class="finalist-evidence">${savedEvidenceCue(home).map(esc).join(" · ")}</p>
      <div class="finalist-next"><span class="eyebrow">YOUR NEXT STEP</span>${move ? `<button class="text-button" data-studio-task="${esc(home.id)}" data-task-target="${esc(move.target)}">${esc(MOVE_ACTIONS[move.target] ?? "Review this record")} ↗</button>` : '<span>Revisit your notes and decide in your own time.</span>'}</div>
      <div class="actions"><button class="button small secondary" data-detail="${esc(home.id)}">Open dossier ↗</button><button class="text-button" data-finalist="${esc(home.id)}" aria-label="Unpin ${esc(home.title)} ${esc(planLabel(home))}">Unpin</button></div></article>`;
...
function bindFinalists() {
  document.querySelectorAll("[data-finalist]").forEach((button)=>{button.onclick=()=>{
    ...
    state.records[id]={...rec,saved:true,finalist:next,...savedSource(id,getHome(id))};
...
function detailDock() {
  return `<nav class="detail-dock" aria-label="Apartment sections"><div>${[["layoutReview","Layout"],["detail-home-evidence","Home criteria"],["detail-costs","Costs"],["tour-draft-count","Tour"],["notes","Notes"],["detail-history","History"],["detail-sources","Sources"]].map(([target,label])=>`<button type="button" data-detail-jump="${target}">${label}</button>`).join("")}</div><button class="button small" type="submit" form="record-form">Save changes</button><button class="dock-close" type="button" data-close aria-label="Close apartment details">Close</button></nav>`;
}
function openJump() {
...
// One composition for both attention surfaces, so SpicyPicks and Focus cannot
// explain the same candidate two different ways. The reasons are the rules that
// actually ran, recomputed from the current preferences on every render -- so a
// reason whose rule stops applying stops being drawn, and nothing is left over
// to read as still true. The open question is the record's own, taken in the
// order openQuestions already ranks them by decision weight. Three reasons at
// most: a wall of chips is not an explanation. Full provenance stays in the
// record, which is one press away, rather than being copied here.
function whyBlock(home,context={}) {
  if (homeSearchActive(prefs)) return `<div class="why-block"><p class="why-label">Selected criteria</p><p>${esc(criteriaStatusText(home))}</p></div>`;
  const reasons=surfacedBecause(home,state,prefs,{...context,feed}).slice(0,3);
  // Reading through the downtown lens adds a question the lens itself makes
  // consequential -- a height nobody recorded -- so a second open fact is worth
  // the room there and nowhere else. Neither slot is ever filled by inventing
  // one: focusUnknowns returns what the record actually leaves open.
  const opens=focusUnknowns(home,record(home.id),prefs,new Date(),doubleDownOn(prefs) ? 2 : 1);
  if(!reasons.length && !opens.length)return "";
  // A layout question's detail IS the evidence label the card already prints
  // beside the plan; repeating it reads as a second, separate piece of
  // evidence. Every other kind carries a caveat that appears nowhere else --
  // that unknown is not none, that a nearby public station is not a resident
  // amenity -- and those are kept.
  const openLine=(open)=>{const caveat=open.kind!=="layout" ? open.detail : "";return `<p class="why-open"><span class="why-open__label">Still open</span> <strong>${esc(open.label)}</strong>${caveat ? ` <span class="meta">${esc(caveat)}</span>` : ""}</p>`;};
  return `<div class="why-block">${reasons.length ? `<p class="why-label">Why it\u2019s here</p><ul class="why-list" aria-label="Why this is here">${reasons.map((reason)=>`<li class="why-item why-item--${esc(reason.kind)}">${esc(reason.text)}</li>`).join("")}</ul>` : ""}${opens.map(openLine).join("")}</div>`;
}
function pickCard(pick,index,lens=pickLens,priority=lens,weights=null) {
...
function renderTradeResults() {
  const anchor=getHome(studio.anchor),options=apartmentTradeoffs(anchor,allHomes(),state,prefs,studio.extra,new Date(),feed);
...
function renderAreas() {
  const areas=areaMatch(allHomes(),state,prefs,studio.bedrooms,studio.areaScope,new Date(),feed);
...
function bindStudioTasks(selector = "[data-studio-task]") {
...
function homeSearchPanel() {
  const active = homeSearchActive(prefs);
  const areas = [...new Set([...downtownAreas, ...prefs.includeAreas, ...prefs.excludeAreas])];
  return `<section class="home-search" aria-labelledby="home-search-title"><div class="home-search-heading"><div><p class="eyebrow">A SEARCH YOU CAN CHANGE</p><h3 id="home-search-title">Downtown home search</h3></div><button class="button ${active ? "secondary" : ""}" id="apply-downtown">${active ? "Reapply starting settings" : "Apply downtown search"}</button></div>
  <${active ? "details" : "div"} class="preset-preview">${active ? '<summary>What the starting settings change</summary>' : ''}<p>Includes River North, Streeterville, Lakeshore East, The Loop and West Loop / Fulton Market; excludes South Loop. Keeps 1–2 bedroom choices. Asks for a private balcony, in-unit washer AND dryer, size &gt;600 sq ft and a recorded high-rise. Excludes documented income-restricted offers; address-level programs need verification.</p><p>Target move-in: Nov 1, 2026. Preferred $2,500–$3,000, flexible: clears the previous minimum, maximum and strict cap. Parking optional; EV not required. Clears text, distance, layout-evidence and single-area filters. Saved homes, notes and named searches stay intact.</p></${active ? "details" : "div"}>
  <p id="home-search-summary" class="home-search-summary" role="status">${esc(homeSearchSummary())}</p>
  <div class="actions"><button class="text-button" id="undo-home-search" ${state.searchUndo ? "" : "disabled"}>Undo last search change</button><button class="text-button" id="reset-home-search">Reset search</button></div>
  <details id="home-search-editor" ${homeSearchControlsOpen ? "open" : ""}><summary>Edit neighborhoods &amp; home criteria</summary>
  <fieldset class="area-choices"><legend>Include any selected area · exclusions always win</legend>${areas.map((n,i) => `<label for="area-${i}"><span>${esc(n)}</span><select id="area-${i}" data-area-choice="${esc(n)}"><option value="any">Any / no preference</option><option value="include" ${prefs.includeAreas.includes(n) && !prefs.excludeAreas.includes(n) ? "selected" : ""}>Include</option><option value="exclude" ${prefs.excludeAreas.includes(n) ? "selected" : ""}>Exclude</option></select></label>`).join("")}</fieldset>
  <p class="meta">Area names come from retained sources. West Loop / Fulton Market is one selectable group of those two source labels. No ZIP, radius or community-area substitution. Unresolved areas stay in verification.</p>
  <div class="home-criteria-controls">${[["privateBalcony","Private balcony"],["inUnitLaundry","In-unit washer AND dryer"],["over600","Strictly greater than 600 sq ft"],["requireHighRise","Recorded high-rise building"],["excludeRestricted","Exclude documented income-restricted offers"]].map(([k,label]) => `<label><input type="checkbox" data-home-criterion="${k}" ${prefs[k] ? "checked" : ""}>${label}</label>`).join("")}<button class="button secondary small" id="laundry-space">Require laundry AND &gt;600 sq ft</button></div>
  <form id="home-budget-form" class="home-budget"><div class="field"><label for="home-move-in">Target move-in</label><input id="home-move-in" type="date" value="${esc(prefs.moveIn)}"></div><div class="field"><label for="preferred-min">Preferred range from</label><input id="preferred-min" type="number" min="0" max="20000" value="${prefs.preferredMin}"></div><div class="field"><label for="preferred-max">Preferred range to</label><input id="preferred-max" type="number" min="0" max="20000" value="${prefs.preferredMax}"></div><div class="field"><label for="home-budget-mode">Budget filtering</label><select id="home-budget-mode"><option value="flexible" ${prefs.budgetMode === "flexible" ? "selected" : ""}>Flexible preferred range</option><option value="strict" ${prefs.budgetMode === "strict" ? "selected" : ""}>Strict min/max in fine-tune filters</option></select></div><div class="field"><label for="home-strict-cap">Optional strict cap in flexible mode</label><input id="home-strict-cap" type="number" min="0" max="20000" placeholder="No cap" value="${prefs.strictCap ?? ""}"></div><button class="button secondary" type="submit">Save range &amp; date</button><p id="home-search-error" role="status"></p></form>
  <p class="meta">These controls filter the records already loaded. Acquisition bounds and provider queries are unchanged. A preferred range never filters out a lower price. Unknown recurring costs are never $0. A property-criteria match is not confirmed available, eligible or within an all-in budget.</p></details></section>`;
}
function homeSearchSummary() {
  if (!homeSearchActive(prefs)) return "Starting settings are unapplied. Your current search remains active.";
  const selected = [[prefs.privateBalcony,"private balcony"],[prefs.inUnitLaundry,"washer + dryer"],[prefs.over600,">600 sq ft"],[prefs.requireHighRise,"high-rise"],[prefs.excludeRestricted,"restricted offers excluded"]].filter(([on])=>on).map(([,s])=>s);
  return `Include: ${prefs.includeAreas.join(", ") || "any area"}. Exclude: ${prefs.excludeAreas.join(", ") || "none"}. ${selected.join(" · ") || "No property requirements"}. ${prefs.budgetMode === "flexible" ? `Preferred ${money(prefs.preferredMin)}–${money(prefs.preferredMax)}; ${prefs.strictCap === null ? "no strict cap" : "strict cap " + money(prefs.strictCap)}` : `Strict ${money(prefs.min)}–${money(prefs.max)}`}. ${prefs.moveIn ? "Target " + prefs.moveIn : "Move-in date not set"}.`;
}
function applyHomePreferences(patch) {
  try {
...
function bindHomeSearch() {
  $("#apply-downtown").onclick = () => applyHomePreferences(downtownSearch(prefs));
  $("#reset-home-search").onclick = resetSearchFilters;
  $("#undo-home-search").onclick = () => {
    if (!state.searchUndo) return;
    const prior = validatePreferences(state.searchUndo); state.searchUndo = null; prefs = prior; persist(); render();
  };
  $("#home-search-editor").ontoggle = e => { if (e.currentTarget?.isConnected) homeSearchControlsOpen = e.currentTarget.open; };
  document.querySelectorAll("[data-area-choice]").forEach(el => el.onchange = () => {
    const name = el.dataset.areaChoice, includeAreas = prefs.includeAreas.filter(n=>n!==name), excludeAreas = prefs.excludeAreas.filter(n=>n!==name);
    if (el.value === "include") includeAreas.push(name);
    if (el.value === "exclude") excludeAreas.push(name);
    applyHomePreferences({homeSearch:true, includeAreas, excludeAreas, neighborhood:"all"});
    [...document.querySelectorAll("[data-area-choice]")].find(el => el.dataset.areaChoice === name)?.focus();
  });
  document.querySelectorAll("[data-home-criterion]").forEach(el => el.onchange = () => {
    const key=el.dataset.homeCriterion; applyHomePreferences({homeSearch:true,[key]:el.checked});
    document.querySelector(`[data-home-criterion="${key}"]`)?.focus();
  });
  $("#laundry-space").onclick = () => applyHomePreferences({homeSearch:true,inUnitLaundry:true,over600:true});
  $("#home-budget-form").onsubmit = e => {
    e.preventDefault();
    if (!e.currentTarget.reportValidity()) return;
    const raw=$("#home-strict-cap").value;
    applyHomePreferences({homeSearch:true,preferredMin:Number($("#preferred-min").value),preferredMax:Number($("#preferred-max").value),budgetMode:$("#home-budget-mode").value,strictCap:raw==="" ? null : Number(raw),moveIn:$("#home-move-in").value});
  };
}
function criteriaStatusText(home) {
  if (!homeSearchActive(prefs)) return "No home-criteria search selected";
  const r=criteriaReading(home,record(home.id),prefs);
  return r.status === "match" ? "Selected property criteria supported; availability, eligibility and total costs still need review" : r.status === "excluded" ? r.excluded.join("; ") : "Confirm: " + r.missing.join("; ");
}
function homeAttributeText(home, key) {
  const r=attributeReading(home,record(home.id),key);
  const words={private:"Private balcony reported",juliet:"Juliet balcony",shared:"Shared facilities",none:"Not provided",unspecified:"Type unspecified",in_unit_both:"Both in-unit appliances reported",washer_only:"Washer only",dryer_only:"Dryer only",hookups:"Hookups only"};
  if(r.status === "supported") return key === "sqft" ? `${r.value} sq ft · plan/unit source` : key === "floor" ? `Floor ${r.value} reported` : words[r.value] ?? String(r.value);
  if(r.status === "conflicting") return "Sources conflict — verify";
  if(r.entries.some(e=>e.applies === "selected")) return "Selected homes advertised · this plan/unit unconfirmed";
  return "Not established for this plan/unit";
}
function homeEvidenceSummary(home) {
  if (!homeSearchActive(prefs) && !homeEvidenceReading(home,record(home.id)).evidence.length) return "";
  const form=buildingForm({...home,home_evidence:homeEvidenceReading(home,record(home.id)).evidence});
  return `<div class="home-evidence-summary"><p class="home-criteria-verdict">${esc(criteriaStatusText(home))}</p><dl>${[["Balcony",homeAttributeText(home,"balcony")],["Laundry",homeAttributeText(home,"laundry")],["Size",homeAttributeText(home,"sqft")],["Building",form.short]].map(([k,v])=>`<div><dt>${k}</dt><dd>${esc(v)}</dd></div>`).join("")}</dl><p class="meta">${esc(moveInReading(home,record(home.id),prefs.moveIn).label)}. Floor and view are separate facts.</p>${homeEvidenceReading(home,record(home.id)).evidence.length ? `<p class="meta">Separate research observed ${esc(homeEvidenceReading(home,record(home.id)).evidence.map(e=>e.source.observed_at).sort().at(-1))}; displayed price keeps its original source date.</p>` : ""}</div>`;
}
function homeEvidenceDetail(home,r) {
  const reading=homeEvidenceReading(home,r);
  const entry=e=>`<article class="home-source-entry"><h4>${esc(e.attribute.replaceAll("_"," "))} · ${esc(e.status)}</h4><p>${esc(e.value === null ? "Not established" : Array.isArray(e.value) ? e.value.join(" / ") : String(e.value).replaceAll("_"," "))}</p><p>${esc(e.note)}</p><p class="meta">${esc(e.scope)}: ${esc(e.subject)} · ${esc(e.applies)} applicability · observed ${esc(e.source.observed_at)}</p>${link(e.source.url,e.source.name+" ↗")}</article>`;
  return `<details class="detail-section home-evidence-detail" id="detail-home-evidence" tabindex="-1"><summary>Home criteria &amp; research <span>Source, date and exact scope</span></summary><p>${esc(criteriaStatusText(home))}</p><p>${esc(homeAttributeText(home,"floor"))} (floor). ${esc(homeAttributeText(home,"view"))} (view). Use Ask next and personal notes to record the visit.</p><p>${esc(preferredBudget(home,r,prefs))}</p><p>No recorded eligibility restriction does not establish unrestricted eligibility.</p><p class="eyebrow">${esc(reading.context)}</p>${reading.evidence.length ? reading.evidence.map(entry).join("") : '<p>No scoped home research recorded. Missing facts are unknown, not a negative finding.</p>'}${r.snapshot ? `<details class="original-home-evidence"><summary>Original saved research (${reading.original.length} assertions)</summary><p>The original snapshot and its source clocks stay unchanged.</p>${reading.original.length ? reading.original.map(entry).join("") : '<p>No home criteria evidence existed in this original snapshot.</p>'}</details>` : ''}${r.home_evidence_update && reading.current && JSON.stringify(reading.current) !== JSON.stringify(r.home_evidence_update.evidence) ? `<details><summary>Separately saved research history</summary>${r.home_evidence_update.evidence.map(entry).join("")}</details>` : ""}${r.home_evidence_update ? `<p class="meta">Later research recorded in this notebook ${esc(recordedLabel(r.home_evidence_update.recorded_at))}. Source observation dates remain separate.</p>` : ''}</details>`;
}
function renderHomeSearchGroups() {
  const target=$("#home-search-groups"); if(!target)return;
  const active=homeSearchActive(prefs); target.hidden=!active;
  const hero=$(".hero .criteria") ?? $(".criteria");
  if(hero) hero.innerHTML=active ? `<span>${money(prefs.preferredMin)}–${money(prefs.preferredMax)} preferred</span><span>${prefs.bedrooms === "all" ? "1–2 bedroom choices" : prefs.bedrooms + " bedroom"}</span><span>Editable home criteria</span>` : '<span>$1,200–$3,000 / month</span><span>1–2 bedrooms</span><span>Parking + EV</span>';
  if(!active){target.replaceChildren();return;}
  $("#home-search-summary").textContent=homeSearchSummary();
  const groups=discoveryGroups(allHomes().filter(h=>!h.notebook_only),state,prefs,feed);
