import {
  ...
  eligibilityReading,
} from "./model.js?v=20260919-eligibility";
...
function renderCard(h) {
  ...
  return `<article class="home-card" data-home="${esc(h.id)}">
    ...
    ${eligibilitySummary(h)}<div class="property-evidence"><p class="meta layout-evidence">${esc(layout.label)}</p><div class="chips">${chip(h, "parking", "Parking")}${chip(h, "charging", "EV")}${formChip(h)}</div>${bandLine(h)}<div class="cost-line"><span>Known monthly subtotal</span><strong>${subtotalValue(c)}</strong></div>${costGap(h, c)}</div>
...
function showDetail(id) {
  ...
  $("#detail-content").innerHTML =
    ...
    ${eligibilitySummary(h, true)}<div class="dossier-subtotal"><span>Known monthly subtotal <strong>${subtotalValue(c)}</strong></span><span>${c.unknown.length ? "Incomplete · " + esc(c.unknown.join(", ")) + " unquoted" : "Every monthly subtotal item entered"} · EV charging separate</span></div>
...
function showCompare() {
  ...
  const rows = [
    ...
    ["Building form on record", (h) => buildingForm(h).short],
...
function refreshSearchCounts() {
  ...
  document.querySelectorAll("[data-search-count]").forEach((span) => {
    ...
    if (search) span.textContent = `${visibleHomes(homes, state, {...search.preferences, utilityEstimate: prefs.utilityEstimate}, feed).length} matches now · ${money(search.preferences.max)} cap`;
  });
}
...
function renderScanCard(home) {
  ...
  return `<article class="home-card scan-card" data-home="${esc(home.id)}"><div class="scan-identity"><p class="neighborhood">${esc(homeCity(home) || home.neighborhood)}</p><h3>${esc(home.title)}</h3><p class="plan-label">${esc(planLabel(home))}</p><p class="address">${esc(home.address)}</p><p class="meta">${esc(layout.label)} · ${home.sqft ? home.sqft + " sq ft reported" : "Size unverified"}</p></div><div class="scan-price"><strong>${money(displayPrice(home))}<small> / mo</small></strong><p class="price-kind">${esc(priceKind(home))}</p><p class="meta">Known subtotal: ${subtotalValue(cost)}</p>${costGap(home, cost)}<p class="meta">${evidence(home)} · ${esc(dateLabel(home.observed_at))}${home.seen_in_latest === false ? " · Not in last area scan" : ""}</p></div>${eligibilitySummary(home)}<div class="scan-actions"><button class="button small secondary" data-detail="${esc(home.id)}">Details</button><button class="save-button" data-save="${esc(home.id)}" aria-label="${rec.saved ? "Remove" : "Save"} ${esc(home.title)} ${esc(planLabel(home))}" aria-pressed="${!!rec.saved}">${heartIcon()}</button><label class="compare-label"><input type="checkbox" data-compare="${esc(home.id)}" ${comparison.has(home.id) ? "checked" : ""}>Compare</label></div></article>`;
}
