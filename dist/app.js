...
function whyBlock(home,context={}) {
  const reasons=surfacedBecause(home,state,prefs,context).slice(0,3);
  const open=headlineUnknown(home,record(home.id),prefs);
  if(!reasons.length && !open)return "";
  ...
  const caveat=open && open.kind!=="layout" ? open.detail : "";
  return `<div class="why-block">${reasons.length ? `<p class="why-label">Why it\u2019s here</p><ul class="why-list" aria-label="Why this is here">${reasons.map((reason)=>`<li class="why-item why-item--${esc(reason.kind)}">${esc(reason.text)}</li>`).join("")}</ul>` : ""}${open ? `<p class="why-open"><span class="why-open__label">Still open</span> <strong>${esc(open.label)}</strong>${caveat ? ` <span class="meta">${esc(caveat)}</span>` : ""}</p>` : ""}</div>`;
}
